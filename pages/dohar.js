...
const dohar = ({ products }) => {
  return (
    //badsheets = doharscover
    //pillow = dohars =< categeory
    <div>
      <section className="text-gray-600 body-font min-h-screen mx-5">
        <div className="container px-5 py-24 mx-auto ">
          <div className="flex flex-wrap  -m-4 justify-center ">
          {Object.keys(products).length===0 && <p> Sorry!all the dohars  are currently out of stock coming soon . Stay tuned! </p>}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "dohars" });
  let dohars = {};
  for (let item of products) {
    if (item.title in dohars) {
      if (
        !dohars[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        dohars[item.title].color.push(item.color);
      }
      if (
        !dohars[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        dohars[item.title].size.push(item.size);
      }
    } else {
      dohars[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        dohars[item.title].color = [item.color];
        dohars[item.title].size = [item.size];
      }
    }
  }
  return {
    props: { products: JSON.parse(JSON.stringify(dohars)) }, // will be passed to the page component as props
  };
}
